...
Vector2 currentPosition(100.f, 100.f); // (100f, 100f) 2D벡터
...
void SoftRenderer::Update2D(float InDeltaSeconds)
{
	...
	// 게임 로직의 로컬 변수
	static float moveSpeed = 100.f; //이동속도
...
void SoftRenderer::Render2D()
{
	...
	//밝은 회색선을 사용해 평행한 벡터를 표현
	static float lineLength = 1.3f;
	Vector2 lineStart = currentPosition * lineLength; //현재 점 좌표에서 *lineLength만큼 곱한길이에서 시작
	Vector2 lineEnd = currentPosition * -lineLength; //현재 점 좌표에서 *-lineLength만큼 곱한길이에서 시작
	r.DrawLine(lineStart, lineEnd, LinearColor::LightGray);

	//벡터를 파란색 픽셀로 표현
	r.DrawPoint(currentPosition, LinearColor::Blue); //점 위치
	//그 외 주변 8칸 픽셀(픽셀이 너무 작아서 시각적으로 더 덩어리져 보이게 하기위해 추가)
